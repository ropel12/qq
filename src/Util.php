...
class Util {
    ...
    private static $_defaulHeaders= array('POST /front/resi HTTP/1.1',
    ...
    {
        self::$Curl = curl_init();
        if($method ==null || $option == null){$header = $url == self::$url["cekongkir"]["cekresi"] ? self::$_defaulHeaders :self::$_jtwilayah;}
        if($method != null){
           match ($method){
            "POST"=> self::SetupPost(),
            "GET"=> self::SetupGet()
           ...
           if($option =='tarif'){
            $header = array("POST /front/tarif HTTP/1.1",
            'Accept: */*',
            'Accept-Language: en-US,en;q=0.9',
            'Connection: keep-alive',
            'Content-Type: application/json',
...
*/  
 ...
 public static function ConvertToJson($data,$option=null,$datasearch =null){
    $log = $GLOBALS["Log"];
    ...
    if($option != 'tarif'){
        if(is_string($err=self::IfEmpty(null,$data["error"]))){return $err;}
        if(isset($data["jenis"])){
       ...
       if(is_string($err=self::IfEmpty(IsExist:null,err:$data["error"]))){return $err;}
...
public static function GetTarif($sender_name,$sender_id,$weight,$dest_name,$dest_id){
   ...
   return self::ConvertToJson($res,'tarif');
    ...
    public static  function IfEmpty(mixed $IsExist =null , bool $err= false)
    {
